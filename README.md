...
| 79 | **🧹 Dead Code Cleanup: مسح زرار السكرول للأعلى (fab-gold)**: حذف من 5 أماكن — `<button>` HTML + call سطر 696 + `function initScrollTop()` من `website.html` + call سطر 16 + function من `script.js` (dead code كامل) + `.fab-gold` + `.fab-gold:hover` من CSS. مش هيتأثر: زرار البحث الأزرق + `.fab-container` + `@keyframes float-fab` | `website.html`, `script.js`, `v3_enhancements.css` |
| 80 | **🧹 مسح fab-top من الهوم + إصلاح تغطية البحث**: مسح `fabTop` من `index.html` (5 أماكن: CSS `.fab-btn:nth-child(2)` + `.fab-top-btn` + HTML `<div class="fab-wrap">` + JS variable + JS toggle). إصلاح تغطية زرار البحث بـ **CSS شرطي** (`body.has-chat-fab .fab-container { bottom: 155px }`) — بدل hard-coded — الصفحات بدون شات مش هتتأثر. `chat-widget.js` بيضيف `has-chat-fab` class تلقائي | `index.html`, `v3_enhancements.css`, `chat-widget.js` |
...
| 21 | 🌐 **Phase 9C-2: شات عام للمنصة** — General Mode يجاوب أسئلة عن المنصة + Cache 5 دقائق + System Prompt يرفض الفتاوى + Fallback ذكي + رسالة ترحيب ديناميكية + أسئلة مقترحة | `routes/chatbot.js`, `public/chat-widget.js`, `browse.html` | ✅ |
| 22 | 🏠 **شات عام في الصفحة الرئيسية** — نفس General Mode widget في `index.html` (الهوم) | `index.html` | ✅ |
...
| #28 | [Performance] | **`sort(() => 0.5 - Math.random())` مش shuffle حقيقي** — بيدي توزيع غير منتظم. الحل الصح: Fisher-Yates Shuffle | Phase 9D — suggestions shuffle |
| #29 | [Performance] | **Constants جوه handler = waste** — أي `Set` أو `Map` ثابتة بتتعمل من جديد كل request؟ انقلها module-level! | Phase 9D — `META_KEYS` كانت جوه handler |
| #30 | [Performance] | **مراجعة الآراء المتعددة = أفضل Quality Gate** — لما تبعت بلان لـ 5 موديلات وتقارن الآراء، بتلاقي ثغرات مكنتش شايفها (Dead Code, False Positives, Diacritics) | Phase 9D — 7 ثغرات اتلقت من 5 موديلات |
...
| #26 | [Performance] | **Cache في المعلومات الثابتة ضروري** — لو عندك DB query بترجع أرقام مش بتتغير كل ثانية (countDocuments)، حط cache بـ TTL (5 دقايق مثلاً) بدل ما تضرب الـ DB كل request | `buildPlatformContext()` في Phase 9C-2 |
| #27 | [Frontend] | **`innerHTML +=` بيكسر HTML tags في typewriter** — لما `typewriterEffect()` بتكتب حرف حرف بـ `el.innerHTML += char` الـ `<a>` tags بتتكسر لأن الـ DOM بيعمل re-parse كل مرة. **الحل:** `else { el.innerHTML = html }` في نهاية الكتابة (re-render مرة واحدة). **فخ إضافي:** الإصلاح الأول كان في أول `type()` بس الـ function مكنتش بتتنادي تاني — لازم يكون في الـ `else` clause تحت `if (i < fullText.length)` | `typewriterEffect()` في `chat-widget.js` سطر 707 |
